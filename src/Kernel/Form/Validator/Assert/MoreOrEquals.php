...
/**
 * Validates that a numeric value is more or equals than expected.
 * Note: null values pass through — use #[NotNull] if null should be rejected.
 */
#[Attribute]
class MoreOrEquals extends AbstractAssert
{
    ...
    public function __construct(float |int $expected,?string $errorMessage = null)
    {
        ...
        $this->errorMessage = $errorMessage ?? "property %s must be more or equals than " . (is_scalar($expected) || $expected === null ? (string)$expected : gettype($expected));
