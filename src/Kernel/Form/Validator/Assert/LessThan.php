 ...
 */
#[Attribute]
class LessThan extends AbstractAssert
{
    ...
    public function __construct(float |int $expected,?string $errorMessage = null)
    {
        ...
        $this->errorMessage = $errorMessage ?? "property %s must be less than " . (is_scalar($expected) || $expected === null ? (string)$expected : gettype($expected));
