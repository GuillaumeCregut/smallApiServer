...
**Location:** `App\Kernel\Security\JwtToken`

#### Token Structure

JWT tokens follow the standard format: `header.payload.signature`

- **Header**: Contains token type (`JWT`) and algorithm (`HS256`)
- **Payload**: Contains user data and metadata (userId, roles, etc.)
- **Signature**: HMAC SHA256 signature for validation

#### Key Methods

##### `createToken(array $payload, string $secret, int $validity = 86400): string`

Creates a new JWT token.

**Parameters:**
- `$payload` (array): Data to encode in token (userId, roles, etc.)
- `$secret` (string): Secret key for signing (from environment)
- `$validity` (int): Token validity duration in seconds (default: 86400 = 1 day)
...
$token = JwtToken::createToken($payload, $secret, 86400);
// Result: eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9...
```

##### `checkToken(string $token, string $secret): bool`

Validates token signature and integrity.
...
```php
$isValid = $jwt->checkToken($token, $secret);
if ($isValid) {
    echo "Token signature is valid";
}
```

##### `checkFormat(string $token): bool`

Validates token format using regex pattern (static method).
...
##### `extractPayload(string $token): array`

Decodes and extracts the payload data from token.

**Parameters:**
- `$token` (string): JWT token

**Returns:** Array containing payload data
...
// Result: ['userId' => 1, 'role' => [...], 'firstname' => 'John', ...]
```
...
### CreateJwtAuth Helper Class

Simplified helper for creating JWT tokens with common payload.

**Location:** `App\Kernel\Security\CreateJwtAuth`

#### Methods

##### `createToken(int $userId, array $role, ?string $firstname, ?string $lastname, ?int $validity = 86400): string`

Creates a JWT token with standard user information.

**Parameters:**
- `$userId` (int): User ID
- `$role` (array): User roles (e.g., ['USER'], ['ADMIN', 'USER'])
...
**Note:** Automatically retrieves secret from environment configuration.
